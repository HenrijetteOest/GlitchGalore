...
func main() {
	...
	go PlaceBid(client, LocalClient)
...
func PlaceBid(client pb.AuctionServiceClient, LocalClient *AuctionClient) {
	for i := 0; i < 30; i++{
		...
		if res.ItemSold == true { 	// If the auction is over, then reset price
			LocalClient.My_Bid = int32(rand.Intn(99)+1) // Random start bid, between 1 and 100
			...
			if res.HighestBid >= LocalClient.My_Bid {
				...
				LocalClient.My_Bid = res.HighestBid + int32(rand.Intn(59)+5) // 1  
