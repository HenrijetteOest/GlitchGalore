...
Our client code includes a function `SendMessageLoop`, that we use to simulate a user sending messsages. It has a loop that repeats 5 times with a message (a random name generated by the same function that generates our username) and a `timer.Sleep` that creates gaps of 0-5 seconds between sending messages. 
